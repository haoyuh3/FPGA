...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    if(dict.find(upper1)==dict.end() || dict.find(upper2)==dict.end()){
        return false;
    }
    auto iter1 = dict.find(upper1);
    auto iter2 = dict.find(upper2);
    vector<string> string1=iter1->second;
    vector<string> string2=iter2->second;
