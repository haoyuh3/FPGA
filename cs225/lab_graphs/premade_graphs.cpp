...
Graph PremadeGraphs::createChinaMap(bool isWeighted){
    ...
    string labels[]
        = {"Xiamen", "Haining", "Shanghai", "Peking",    "Tibet",
           "Chengdu",     "Hainan",  "Shenzhen",   "Taiwan", "Shenyang"};
...
vector<Vertex> vertices(numCities);
    ...
    g.insertEdge(vertices[0], vertices[8]);//xia-taiwan
    g.insertEdge(vertices[0], vertices[7]);//xia-shenzhen
    g.insertEdge(vertices[0], vertices[1]);//xia-haining
    g.insertEdge(vertices[1], vertices[2]);//haing-shanghai
    g.insertEdge(vertices[2], vertices[3]);//shanghai-peking
    g.insertEdge(vertices[2], vertices[9]);//shanghai-shenyang
    g.insertEdge(vertices[3], vertices[5]);//peking-chengdu
    g.insertEdge(vertices[3], vertices[7]);//peking-shenzhen
    g.insertEdge(vertices[3], vertices[8]);//peking - tai wan
    g.insertEdge(vertices[4], vertices[5]);//tibet-chengdu
    g.insertEdge(vertices[0], vertices[5]);//cehngdu xiamen
    g.insertEdge(vertices[5], vertices[7]);//chengdu-shenzhen
    g.insertEdge(vertices[6], vertices[7]);//hainan - shen zhen
    g.insertEdge(vertices[7], vertices[8]);//shen-taiwan
    g.insertEdge(vertices[5], vertices[8]);//taiwan-chengdu
    g.insertEdge(vertices[5], vertices[9]);//shenyang-chengdu
    g.insertEdge(vertices[2], vertices[7]);//shenzhen-shanghai
    g.insertEdge(vertices[3], vertices[6]);//hainan-peking

    if (isWeighted) {
        g.setEdgeWeight(vertices[0], vertices[8], 118);//
        g.setEdgeWeight(vertices[0], vertices[7], 359);//
        g.setEdgeWeight(vertices[0], vertices[1], 445);//
        g.setEdgeWeight(vertices[1], vertices[2], 68);//
        g.setEdgeWeight(vertices[2], vertices[3], 649);//
        g.setEdgeWeight(vertices[2], vertices[9], 736);//
        g.setEdgeWeight(vertices[3], vertices[5], 940);//
        g.setEdgeWeight(vertices[3], vertices[7], 1202);//
        g.setEdgeWeight(vertices[3], vertices[8], 1078);//2
        g.setEdgeWeight(vertices[4], vertices[5], 775);//
        g.setEdgeWeight(vertices[0], vertices[5], 951);//
        g.setEdgeWeight(vertices[5], vertices[7], 824);//
        g.setEdgeWeight(vertices[6], vertices[7], 302);//
        g.setEdgeWeight(vertices[7], vertices[8], 481);//
        g.setEdgeWeight(vertices[5], vertices[8], 1136);//
        g.setEdgeWeight(vertices[5], vertices[9], 1324);//
        g.setEdgeWeight(vertices[2], vertices[7], 745);//
        g.setEdgeWeight(vertices[3], vertices[6], 1415);//
    }
