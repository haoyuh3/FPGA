                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if((a->freq.getFrequency())<(b->freq.getFrequency())){
        singleQueue.pop();
        smallest=a;
    }else{
        mergeQueue.pop();
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     for(unsigned long i=0; i<frequencies.size();i++){
        TreeNode* a=new TreeNode(frequencies[i]);
        singleQueue.push(a);
     }
     while(singleQueue.size()+mergeQueue.size()!=1 ){
        ...
        TreeNode* mergeN=new TreeNode(small_1->freq.getFrequency()+small_2->freq.getFrequency());
        mergeN->left=small_1;
        mergeN->right=small_2;
        mergeQueue.push(mergeN);
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if(!bfile.hasBits()){
        ...
        Frequency* new_f=new Frequency(bfile.getNextByte(),0);
        new_node=new TreeNode(*new_f);

    }
    else{
        Frequency* f=new Frequency(0);
        new_node=new TreeNode(*f);
