...
//     if(split_d<=radius){
//       if(signal==true){//we have gone to left 
//        Point<Dim> otherway=findNearestNeighbor(subroot->right,query,(dim+1)%Dim);

//       }else{
//         Point<Dim> otherway=findNearestNeighbor(subroot->left,query,(dim+1)%Dim);
//       }
//     if(shouldReplace(query,cur,otherway)){
//       cur=otherway;
//     }
//     }
//     return cur;
