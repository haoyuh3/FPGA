...
class KDTree
{
  ...
  private:
     ...
     */
    unsigned quickselect(vector<Point<Dim>>& list_, int dim, unsigned left, unsigned right, unsigned pivotIndex);

    Point<Dim>& quickselect2(vector<Point<Dim>>& list_, int dim, unsigned left, unsigned right, unsigned k);

    KDTreeNode* helperfun(vector<Point<Dim>>& list_,int dim, unsigned left, unsigned right);

    void copy(KDTreeNode* node, KDTreeNode* other);
