...
size_t binarysearch(const std::vector<T>& elements, const C& val, unsigned start, unsigned end){
    if(start==end){
        if(val>elements[end]){return end+1;}
        else return end;
    }
    size_t mid = (start + end) / 2;
    if((val > elements[mid] && val < elements[mid + 1])){return mid + 1;} 
    if(val < elements[mid]){ return binarysearch(elements, val, start, mid);}
    else if(val > elements[mid]){
        return binarysearch(elements, val, mid + 1, end);
    }
    else return mid;
}
template <class T, class C>
