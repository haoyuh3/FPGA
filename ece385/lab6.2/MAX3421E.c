...
XTmrCtr Usb_timer;
//  0x00000001
static u32 SlaveDeselectMask = 0x00000001; 
static u32 SlaveselectMask1  = 0x00000000;
...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	int writeStatus;
	u8 writeBuffer[2]; // a two byte buffer
	//select MAX3421E00
	//xil_printf ("reached MAXreg_wr function\n");
	XSpi_SetSlaveSelectReg(&SpiInstance, SlaveselectMask1); // 0x000000000
	...
	XSpi_SetSlaveSelectReg(&SpiInstance, SlaveDeselectMask); //0x00000001
	return;
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	xil_printf ("reached MAXbytes_wr function\n");
	...
	int Status1=XSpi_Transfer(&SpiInstance,writebuffer,NULL,nbytes+1);
	if(Status1!=0){
		xil_printf ("error");
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0x00000000);
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	xil_printf ("readched MAXbytes_rd\n");
	XSpi_SetSlaveSelectReg(&SpiInstance, SlaveselectMask1); //tells us if the slave can be reached
	XSpi_SetSlaveSelect(&SpiInstance, 0x00000001);
    writebuffer[0] = reg;
	readbuffer[0] = 0;
...
//	}
	...
	int Status1=XSpi_Transfer(&SpiInstance, writebuffer, readbuffer, 1+nbytes);
	if(Status1!=0){
		xil_printf ("error");
	}
	else{
